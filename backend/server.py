...
async def upload_video(file: UploadFile = File(...), request: Request = None):
    ...
    fid = str(uuid.uuid4()); ext = Path(file.filename or "vid.mp4").suffix or ".mp4"; sp = VIDEO_DIR / f"{fid}{ext}"
    ...
    dur = get_video_duration(str(sp))
    return {"file_id": fid, "file_path": str(sp), "file_type": "video", "size_mb": round(len(content)/(1024*1024), 2), "duration": round(dur, 1)}
...
async def upload_audio(file: UploadFile = File(...)):
    ...
    content = await file.read()
    if len(content)/(1024*1024) > 50: raise HTTPException(status_code=400, detail="Max 50MB")
    with open(sp, "wb") as f: f.write(content)
    return {"file_id": fid, "file_path": str(sp)}

@api_router.post("/extract-frames")
async def extract_frames(file: UploadFile = File(...)):
    """Extract key frames and transcribe audio from a reference video"""
    fid = uuid.uuid4().hex
    ext = Path(file.filename or "video.mp4").suffix or ".mp4"
    sp = UPLOAD_DIR / f"ref_{fid}{ext}"
    content = await file.read()
    with open(sp, "wb") as f: f.write(content)
    # Extract 4 frames at equal intervals
    dur_r = subprocess.run(["/usr/bin/ffprobe", "-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", str(sp)], capture_output=True, text=True, timeout=10)
    duration = float(dur_r.stdout.strip()) if dur_r.stdout.strip() else 10.0
    frames = []
    for i in range(4):
        ts = (duration / 5) * (i + 1)
        fn = f"frame_{fid}_{i}.jpg"
        fp = UPLOAD_DIR / fn
        subprocess.run(["/usr/bin/ffmpeg", "-y", "-ss", str(ts), "-i", str(sp), "-vframes", "1", "-q:v", "3", str(fp)], capture_output=True, timeout=10)
        if fp.exists():
            frames.append({"index": i, "url": f"/api/serve-file/{fn}", "timestamp": round(ts, 1)})
    # Extract audio and transcribe using Gemini 2.5 Flash (with speaker diarization)
    transcript = ""; diarized = []
    audio_path = UPLOAD_DIR / f"ref_audio_{fid}.mp3"
    subprocess.run(["/usr/bin/ffmpeg", "-y", "-i", str(sp), "-vn", "-acodec", "libmp3lame", "-q:a", "4", str(audio_path)], capture_output=True, timeout=30)
    if audio_path.exists() and audio_path.stat().st_size > 1000:
        try:
            ...
            audio_attach = FileContentWithMimeType(file_path=str(audio_path), mime_type="audio/mpeg")
            msg = UserMessage(text="Transcribe and diarize this audio as JSON only.", file_contents=[audio_attach])
            ...
            try:
                ...
                logger.info(f"Transcribed+diarized ref video ({audio_path.stat().st_size}b) -> {len(transcript)} chars, {len(diarized)} segs, speakers={len(set(s.get('speaker',1) for s in diarized))}")
            except Exception as je:
                # Fallback: treat raw output as plain transcript
                transcript = raw
                logger.warning(f"Diarization JSON parse failed ({je}); using plain transcript ({len(transcript)} chars)")
        except Exception as e:
            logger.warning(f"Transcription failed: {e}")
    return {"video_path": str(sp), "duration": round(duration, 1), "frames": frames, "frame_count": len(frames), "transcript": transcript, "diarized_segments": diarized}
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(120.0)) as c:
            with open(sp, "rb") as fh:
                ...
                data = {"model": "whisper-1", "language": "hi", "response_format": "verbose_json"}
                r = await c.post("https://api.openai.com/v1/audio/transcriptions", headers={"Authorization": f"Bearer {EMERGENT_LLM_KEY}"}, files=files, data=data)
            if r.status_code == 200:
                result = r.json()
                return {"transcript": result.get("text", ""), "segments": result.get("segments", []), "file_path": str(sp)}
            else:
                return {"transcript": "", "segments": [], "file_path": str(sp), "error": "Transcription failed"}
    except Exception as e:
        logger.error(f"Transcription failed: {e}")
        return {"transcript": "", "segments": [], "file_path": str(sp), "error": str(e)}

@api_router.post("/merge-segments/{project_id}")
async def merge_segments(project_id: str, request: Request = None):
    user = await get_current_user(request)
    project = await db.video_projects.find_one({"id": project_id, "user_id": user["user_id"]}, {"_id": 0})
    if not project: raise HTTPException(status_code=404, detail="Project not found")
    segments = project.get("result_segments", [])
    if len(segments) < 2: raise HTTPException(status_code=400, detail="Need 2+ segments to merge")
    try:
        merge_dir = UPLOAD_DIR / f"merge_{uuid.uuid4().hex}"
        merge_dir.mkdir(exist_ok=True)
        seg_files = []
        async with httpx.AsyncClient(timeout=httpx.Timeout(120.0), follow_redirects=True) as c:
            for i, seg in enumerate(segments):
                ...
                if resp.status_code == 200:
                    ...
                    with open(fp, "wb") as f: f.write(resp.content)
                    seg_files.append(fp)
        if len(seg_files) < 2: raise Exception("Could not download enough segments")
        list_path = merge_dir / "list.txt"
        with open(list_path, "w") as f:
            for sf in seg_files: f.write(f"file '{sf}'\n")
        out_path = UPLOAD_DIR / f"merged_{uuid.uuid4().hex}.mp4"
        result = subprocess.run(["/usr/bin/ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(list_path), "-c", "copy", str(out_path)], capture_output=True, timeout=120)
        if result.returncode != 0:
            result = subprocess.run(["/usr/bin/ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(list_path), "-c:v", "libx264", "-c:a", "aac", str(out_path)], capture_output=True, timeout=120)
        if not out_path.exists(): raise Exception("FFmpeg merge failed")
        merged_url = f"/api/serve-file/{out_path.name}"
        await db.video_projects.update_one({"id": project_id}, {"$set": {"merged_url": merged_url, "updated_at": datetime.now(timezone.utc).isoformat()}})
        import shutil
        shutil.rmtree(merge_dir, ignore_errors=True)
